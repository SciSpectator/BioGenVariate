...
GenVariate is a sophisticated, GUI-driven Python application designed for the in-depth analysis of gene expression data from the Gene Expression Omnibus (GEO) and custom user datasets. It provides a multi-step workflow to extract, filter, label, and analyze biological samples and experiments from data. The application moves beyond simple keyword searches by integrating a local AI agent, powered by gemma2:9b model, for automated metadata classification.

The suite is engineered for researchers who need to identify relevant experimental cohorts from the vast GEO database, classify samples using either AI-assisted or manual methods, and explore the descriptive metadata associated with unique gene expression patterns. It excels at analyzing user-defined regions of interest within a gene's expression distribution and generating insightful visualizations and data tables.



<img width="1024" height="963" alt="Screenshot from 2025-10-16 12-22-29" src="https://github.com/user-attachments/assets/da07e607-490e-478a-821e-6f35c3972ca5" />
...
How to Use It
    ...
    The process begins immediately in the background. The workers simultaneously send samples to the AI agent, which reads the full metadata for each sample (including title, source, characteristics, and protocols).
...
<img width="354" height="438" alt="Screenshot from 2025-10-20 00-52-44" src="https://github.com/user-attachments/assets/c899f516-2ca8-4422-b0be-07cdb9a136f2" />
...
You can customize the colors of the plots for better visualization ( like also in 3.Show Gene Distribution results plots)
...
Interactivity: You can click on a GSE ID in the legend to open its official page on the NCBI GEO website. You can also click the colored patch next to it to change its color on the plot (_Ryc10._) - like in 2. Compare Distributions. This helps you quickly identify which experiments contributed the most samples to your selected expression range.
